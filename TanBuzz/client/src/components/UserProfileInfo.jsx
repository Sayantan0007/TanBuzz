import { Calendar, MapPin, PenBox, Verified } from 'lucide-react'
import React from 'react'
import moment, { } from 'moment'

const UserProfileInfo = ({ profileData, posts, profileId, setShowEdit }) => {
    // console.log(profileData)
    return (
        <div className='relative py-4 px-6 md:px-8 bg-white'>
            <div className='flex flex-col md:flex-row items-start gap-6'>
                <div className='w-32 h-32 border-4 border-white shadow-lg absolute -top-16 rounded-full'>
                    <img src={profileData.profile_picture} alt="" className='rounded-full absolute z-2' />
                </div>
                <div className='w-full pt-16 md:pt-0 md:pl-36'>
                    <div className='flex flex-col md:flex-row items-start justify-between'>
                        <div>
                            <div className='flex items-center gap-3'>
                                <h1 className='text-2xl font-bold text-gray-900'>{profileData.full_name}</h1>
                                <Verified className='h-6 w-6 text-blue-500' />
                            </div>
                            <p className='text-gray-500'>{profileData.username ? `@${profileData.username}` : `Add a username`}</p>
                        </div>
                        {/* If user is not on others profile that means he is opening his profile so we will give edit button */}
                        {
                            !profileId && (
                                <button onClick={() => setShowEdit(true)} className='cursor-pointer flex items-center justify-center border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg font-medium transition-colors mt-4 md:mt-0'>
                                    <PenBox className='h-4 w-4 ' />
                                    <p className='text-sm '>Edit</p>
                                </button>
                            )
                        }
                    </div>
                    <p className='text-gray-700 text-sm max-w-md mt-4'>
                        {profileData.bio}
                    </p>
                    <div className='flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4'>
                        <span className='flex items-center gap-1.5'>
                            <MapPin className='h-4 w-4' />
                            {profileData.location ? profileData.location : "Add location"}
                        </span>
                        <span className='flex items-center gap-1.5'>
                            <Calendar className='h-4 w-4' />
                            Joined <span className='font-medium'>{moment(profileData.createdAt).fromNow()} </span>
                        </span>
                    </div>
                    <div className='flex items-center gap-6 mt-6 border-t border-gray-200 pt-4'>
                        <div>
                            <span className='sm:text-xl font-bold text-gray-900'>{posts.length}</span>
                            <span className='text-xs sm:text-sm text-gray-500 ml-1.5'>Posts</span>
                        </div>
                        <div>
                            <span className='sm:text-xl font-bold text-gray-900'>{profileData.followers.length}</span>
                            <span className='text-xs sm:text-sm text-gray-500 ml-1.5'>Followers</span>
                        </div>
                        <div>
                            <span className='sm:text-xl font-bold text-gray-900'>{profileData.following.length}</span>
                            <span className='text-xs sm:text-sm text-gray-500 ml-1.5'>Following</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default UserProfileInfo
